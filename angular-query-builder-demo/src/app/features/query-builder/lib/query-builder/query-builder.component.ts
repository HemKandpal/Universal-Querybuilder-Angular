...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  private defaultTemplateTypes: string[] = [
    'string', 'number', 'time', 'date', 'category', 'boolean', 'multiselect', 'json_path', 'multi_level'];
  private defaultPersistValueTypes: string[] = [
    'string', 'number', 'time', 'date', 'boolean', 'json_path', 'multi_level'];
    ...
    if (type === 'object') {
      ...
      if (config.entities) {
        this.entities = Object.keys(config.entities).map((value) => {
          config.entities = {};
          let entity = config.entities[value];
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    ...
    const ruleErrorStore: string | any[] = [];
    let hasErrors = false;
  ...
  getField(filedValue: string): Field | undefined {
    let fields = this.fields.filter((field) => {
      return field && field.value === filedValue
    });
    return fields.length > 0 ? fields[0] : undefined;
  }


  getInputType(field: string | any, operator: string | any): string {
    ...
    let operatirAlias = operator ? operator.replace(/\s/g, "").toUpperCase() : null;
    switch (operatirAlias) {
      case 'ISNULL':
      case 'ISNOTNULL':
      case 'EXISTS':
      case 'DOESNOTEXIST':
  ...
  getClassNames(...args: string[] | any): string | any {
    const clsLookup = this.classNames ? this.classNames : this.defaultClassNames;
    const classNames = args.map((id: string) => clsLookup[id as keyof typeof clsLookup] || this.defaultClassNames[id as keyof typeof this.defaultClassNames]).filter((c: any) => !!c);
    return classNames.length ? classNames.join(' ') : null;
  }

  getDefaultField(entity: Entity): Field | any {
    if (!entity) {
      return null;
  ...
  getDefaultOperator(field: Field): string | any {
  ...
  coerceValueForOperator(operator: string | any, value: any, rule: Rule): any {
  ...
  changeField(fieldValue: any, rule: Rule): void {
    rule.fieldInfo = this.getField(fieldValue);
    rule.path = fieldValue;
    if (!rule.fieldInfo.children || rule.fieldInfo.children.length < 1) {
      rule.subField = undefined;
    } else {
      rule.subField = rule.fieldInfo.children[0].value;
    }
    this._changeField(fieldValue, rule);
  }

  changeSubField(fieldValue: string, rule: Rule): void {
    rule.path = rule.field + "." + fieldValue;
    this._changeField(fieldValue, rule);
  }

  _changeField(fieldValue: string, rule: Rule): void {
    if (this.disabled) {
      return;
    }
  ...
  changeEntity(entityValue: string, rule: Rule, index: number, data: RuleSet): void | any {
    if (this.disabled) {
      return;
    }
    let i = index;
    let rs = data;
    const entity: Entity | any = this.entities.find((e) => e.value === entityValue);
  ...
  getButtonGroupContext(): ButtonGroupContext | any {
  ...
  getRemoveButtonContext(rule: Rule): RemoveButtonContext | any {
  ...
  getFieldContext(rule: Rule): FieldContext | any {
  ...
  getEntityContext(rule: Rule): EntityContext | any {
  ...
  getOperatorContext(rule: Rule): OperatorContext | any {
  ...
  getInputContext(rule: Rule): InputContext | any {
  ...
  private checkEmptyRuleInRuleset(ruleset: RuleSet): boolean {
    if (!ruleset || !ruleset.rules || ruleset.rules.length === 0) {
      ...
      return ruleset.rules.some((item: any) => {
        if (item.rules) {
