...
function buildEsRule(
   ...
   * */
  let parameters;
  fieldName = fieldSrc ? fieldSrc : fieldName;
...
function buildEsGroup(
  ...
  const modifiedResultFlat = buildNestedQueries(
    resultFlat,
    fieldTypeMap,
    occurrence
  );
...
function buildNestedQueries(resultFlat, fieldTypeMap, occurrence) {
  let pathMap = {};
  let newResultFlat = [];
  resultFlat.forEach((item) => {
    for (let key in item) {
      for (let field in item[key]) {
        if (fieldTypeMap[field] === "json_path") {
          let splitedPath = field.split(".");
          if (splitedPath && splitedPath.length > 0) {
            let tempQuery = JSON.parse(JSON.stringify(item));
            if (pathMap[splitedPath[0]]) {
              pathMap[splitedPath[0]].push(tempQuery);
            } else {
              pathMap[splitedPath[0]] = [tempQuery];
            }
          }
        } else {
...
export function elasticSearchFormat(
  ...
  if (type === "rule" && properties.get("field")) {
    ...
    fieldTypeMap[fieldSrc] = _valueType;
    ...
    if (value && Array.isArray(value[0])) {
      ...
      return value[0].map((val) =>
        buildEsRule(field, [val], operator, config, valueSrc, syntax, fieldSrc)
      );
    } else {
      return buildEsRule(
        field,
