...
const convertFromLogic = (
  ...
  if (isLocked) {
    ...
    ret =
      convertFieldRhs(op, vals, conv, config, not, meta, parentField) ||
      convertFuncRhs(
        op,
        vals,
        conv,
...
const convertValRhs = (val, fieldConfig, widget, config, meta) => {
  if (val === undefined) val = fieldConfig?.defaultValue;
...
const convertFieldRhs = (
  ...
  if (conv.varKeys.includes(op) && typeof vals[0] == "string") {
    let field = normalizeField(config, vals[0], parentField);
    let valueSrc = "field";
    if (op === "dynamic") {
      valueSrc = field;
      field = "json_path";
...
const convertLhs = (
  ...
  const _parse = (k, v) => {
    return (
      convertFieldLhs(k, v, conv, config, not, meta, parentField) ||
...
const convertFuncRhs = (
  ...
  if (funcKeys.length) {
    ...
    for (const [f, fc] of iterateFuncs(config)) {
      if (
        fc.jsonLogicImport &&
        (returnType ? fc.returnType == returnType : true)
      ) {
        let parsed;
        try {
      ...
      if (v?.type == "group" || v?.type == "rule_group") {
        ...
        if (!groupField) {
          ...
          const isInParent = shallowEqual(
            parentFieldParts,
            groupPath.slice(0, parentFieldParts.length)
          );
...
const _parseRule = (
  ...
  if (!lhs) return;
  const { field, fieldSrc, having, isGroup, args } = lhs;
  const fieldConfig = getFieldConfig(config, field);
  if (!fieldConfig) {
